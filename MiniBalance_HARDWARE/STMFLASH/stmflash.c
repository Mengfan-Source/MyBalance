...
#define FLASH_START_ADDR  0X0800C000 	//设置FLASH 保存地址(必须为偶数，且其值要大于本代码所占用FLASH的大小+0X08000000)
//解锁STM32的FLASH
...
{
	STMFLASH_Read(FLASH_START_ADDR,(u16*)Start_Read,2);	
	if(PID_Parameter[0]==65535&&PID_Parameter[1]==65535)
	Flag_Zhongzhi=0;	
	else
	Flag_Zhongzhi=Start_Read[0];
}	
/**************************************************************************
函数功能：向Flash写入指定数据
入口参数：无
...
{
Start_Write[0]=Flag_Zhongzhi;
STMFLASH_Write(FLASH_START_ADDR,(u16*)Start_Write,2);	
}	
